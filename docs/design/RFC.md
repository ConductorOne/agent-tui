...
**Exit-code lifecycle (fate-fidelity).** A pane's exit code is first-class and faithful across every observer. The code is shell-style — normal exit → code; signal death → **128+signal** (SIGTERM→143, SIGKILL→137) — derived in `shell_exit_code`: portable-pty's own child collapses `.exit_code()` to 1 on a signal death but still reports the signal via `.signal()` (a `strsignal` name), which we reverse to a number with the same `strsignal` table; the `StdChildShim` (pipe/closed-stdin) path already encodes 128+sig. When a child dies for **any** reason (self-exit, `die`, or a `die` from a *third* client), the pane becomes **terminal-retained**: it stays in the registry with the code remembered (`Pane.last_exit`) instead of being removed. Consequences: (1) every mid-stream follower (`tail --follow` / `watch` / `attach`) receives a faithful terminal `eof{exit_code}`, even when a third client initiated the death; (2) a **late** observer that connects after exit gets the remembered code (an immediate `eof{exit_code}`, and for `attach` the final frame), not "no such pane"; (3) `list` surfaces `exit_code` for retained panes; (4) the `watch` / `attach` / `tail --follow` **CLI process exit status mirrors the child** so a supervisor reads the wrapped task's true fate off the wrapper. Retained panes persist for late observers until the session is GC'd / the daemon restarts (one-shot commands keep their current semantics).
